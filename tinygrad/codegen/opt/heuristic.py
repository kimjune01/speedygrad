...
def hand_coded_optimizations(k:Scheduler) -> Scheduler:
  ...
  tc_opt -- controls which kinds of kernels may be eligible for tensor cores application (default 2 during SEARCH, 0 otherwise)
  ...
  if resolve(prod(k.output_shape[i] for i in k.upcastable_dims) <= (240 if NOLOCALS else 2048), False):
    for axis, sz in itertools.product((0, 1, 2), (32,)):
      try:
