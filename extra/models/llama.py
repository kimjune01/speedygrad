...
class Transformer:
  ...
  def __call__(self, tokens:Tensor, start_pos:int, temperature:float=0.0, top_k:int=0, top_p:float=0.8, alpha_f:float=0.0, alpha_p:float=0.0):
    # Single-token forward (decode + per-token prefill) goes through the JIT'd cuGraph path.
    # start_pos=0 is included: the symbolic Variable's domain covers [0, max_context-1] so the
    # JIT'd kernel handles the first token as well as later ones — eliminates the cold-call
    # overhead that previously made prefill scale at non-JIT cost.
    if tokens.shape[0:2] == (1,1) and self.forward_jit is not None:
      return self.forward_jit(tokens, Variable("start_pos", 0, self.max_context-1).bind(start_pos), temperature, top_k, top_p, alpha_f, alpha_p)
