"""
torch + HF transformers baseline: Llama 3.2 1B Instruct fp16 decode tok/s.

Apples-to-apples comparator for speedygrad's `examples/llama3.py --benchmark --size 1B`.

Methodology matches speedygrad's bench:
  - One prefill pass over the chat-template prompt
  - N decode tokens (default 20), one at a time, past_key_values cached
  - Each decode token timed individually with cuda.synchronize before/after
  - p10/p50/p90 reported across the per-token measurements (first token excluded as warmup)
  - Multiple full runs (default 3) to check stability
"""
...
def main():
  ...
  ap.add_argument("--runs", type=int, default=3, help="full prefill+decode runs")
  ...
  all_decode_us = []  # per-token decode latencies across all runs (excluding first per run)
  prefill_ms_list = []
  ...
  with torch.no_grad():
    for r in range(args.runs):
      ...
      if generated_text_first is None:
        generated_text_first = tok.decode(decoded_ids, skip_special_tokens=True)
  ...
  result = {
    "model": MODEL_ID, "device": torch.cuda.get_device_name(0), "dtype": "fp16",
